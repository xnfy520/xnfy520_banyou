Ext.define('Xnfy.controller.ClassifyMenu', {
    ...
    openCommodityManage : function(panel,id,obj){
        ...
        if (tab) {
            ...
            panel.id = n;
            panel.data = obj.raw;
            var gridpanel = panel.child('treepanel');
            gridpanel.setRootNode({
                id:obj.internalId,
                name:'root',
                expanded: true
            });
            // var pagingtoolbar = gridpanel.getDockedItems('pagingtoolbar')[0];
            // gridpanel.store = Ext.create('Xnfy.store.CommodityList').load({scope:this,params:{category:obj.internalId},callback:function(records,operation,success){
            // }});
            // gridpanel.reconfigure(store);
            // pagingtoolbar.bindStore(store);
            center.setActiveTab(center.add(panel));
