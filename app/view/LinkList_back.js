                    ...
                    items:[{
                        ...
                        },
                        {
                                    ...
                                    itemclick: function(list, record, eml, index,a,b) {
                                        if(index==1){
                                            ...
                                            Ext.create('Ext.window.Window', {
                                                id:imgwindow,
                                                title: '选择图片',
                                                custom_variables:{image_id:'link-image',filed_id:'link-image-field'},
                                                maximizable:false,
                                                resizable:false,
                                                modal:true,
                                                constrain: true,
                                                ...
                                                listeners:{
                                                    scope:this,
                                                    close:function(){
                                                        if(this.up('form').getForm().findField('image').value==''){
                                            ...
                                            this.up('form').getForm().findField('image').setValue('');
                                        }
                                    }
                                }
                            },
                            listeners: {
                                ...
                                change:function(combo,n,o){
                                    if(n==0){
                                        var img_cover = combo.up('form').queryById('link-image');
                                        img_cover.setSrc('');
                    ...
                    listeners:{
                        collapse:function(p){
                            if(p.getForm().findField('id')){
                                // this.ownerCt.child('gridpanel').down('[dataIndex=indexing]').setVisible(true);
                                this.ownerCt.child('gridpanel').getDockedItems('toolbar [itemId=edit]')[0].setDisabled(false);
                            }
                        },
                        expand:function(p){
                            // this.ownerCt.child('gridpanel').down('[dataIndex=indexing]').setVisible(false);
